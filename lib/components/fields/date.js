...
module.exports = function() {
  return merge.recursive(Field(), {
    ...
    ready: function() {
      ...
      if (!this.value) {
        options.startDate = moment().format(this.format);
        options.endDate = moment().format(this.format);
      }
    ...
    methods: {
      momentizeValue: function() {
       if (isDateString(this.value))
        this.value = this.range?
      {start:moment(this.value.start, DATE_FORMAT),
        end:moment(this.value.end, DATE_FORMAT)}:
        moment(this.value, DATE_FORMAT);
      },
      setNow: function (e) {
        e.stopPropagation();
      ...
      setValue: function(val) {
        ...
        this.datepicker.data('daterangepicker').setEndDate(val.format(this.format));
        this.datepicker.trigger("change");
    ...
    computed: {
      ...
      formattedDate: {
        get:function() {
          if (!this.value ||
            (!this.range && (!this.value.format || this.value.format()=='Invalid date')) ||
            (this.range &&
        ...
        set:function(val) {
          ...
          if (!this.range && !moment(val, this.format).isValid()) return;
